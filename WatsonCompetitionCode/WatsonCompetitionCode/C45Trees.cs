...
namespace WatsonCompetitionCode
{
    class C45Trees
    {
        ...
        public C45Trees(string filename)
        {
            decisionTree = Accord.MachineLearning.DecisionTrees.DecisionTree.Load(filename);
        }

        public C45Trees(Dictionary<int, Canidate> canidates)
        {
            ...
            var learning = new C45Learning(decisionTree);
        ...
        {
            decisionTree.Save(filename);
        }


        public Boolean runTreeOnInput(Int32[] values)
        {
            int decision = decisionTree.Compute(values);
            if (decision == 1)
            {
                return true;
            }
